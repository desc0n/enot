...
class Model_Mail extends Kohana_Model
{
     ...
     */
    public function __construct($serverPath, $port = 993, $service = 'imap')
    {
     ...
     */
    public function search($criteria = 'ALL', $limit = null, $checkMail = null, ArrayCollection $checkUidArr)
    {
        $messages = new MailMessageCollection();
        ...
        if ($results = imap_search($this->getImapStream(), $criteria, SE_UID)) {
            ...
            foreach ($results as $messageId) {
                if ($checkMail == $this->username && !$checkUidArr->contains(['uid' => $messageId])) {
     ...
     * @return ArrayCollection
     */
    public function getMessages($limit = null, $checkMail = null, ArrayCollection $checkUidArr)
    {
        ...
        for ($i = $numMessages; $i >= 1; $i--) {
            ...
            if ($checkMail == $this->username && !$checkUidArr->contains(['uid' => $uid])) {
                $messages->add(new Message($uid, $this));
            }
        }
